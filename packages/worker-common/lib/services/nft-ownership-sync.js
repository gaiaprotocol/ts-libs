...
async function fetchAndStoreNftOwnershipRange(env, client, nftAddress, start, end) {
  ...
  `;
    ...
    const ownerships = await Promise.all(Array.from({ length: end - start + 1 }, (_, idx) => {
    ...
    const statements = ownerships.map(({ tokenId, owner }) => env.DB.prepare(upsertSql).bind(nftAddress, tokenId, owner));
  ...
  `;
    ...
    for (const log of logs) {
        ...
        if (range && tokenId >= range.start && tokenId <= range.end) {
            statements.push(env.DB.prepare(upsertHolderSql).bind(nftAddress, tokenId, to));
        }
    }
    // 2. Update sync checkpoint
    statements.push(env.DB.prepare(`UPDATE contract_event_sync_status
       SET last_synced_block_number = ?, last_synced_at = strftime('%s','now')
       WHERE contract_type = ?`).bind(Number(toBlock), 'ERC721'));
