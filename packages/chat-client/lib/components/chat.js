...
import { chatProfileService } from '../services/chat-profile';
...
function makeActivatable(elm, onActivate) {
    elm.style.cursor = 'pointer';
    elm.setAttribute('role', 'button');
    elm.setAttribute('tabindex', '0');
    const handler = (ev) => onActivate(ev);
...
function createChatComponent({ roomId, myAccount, useAddressAvatar, onProfileClick }) {
    // 상태
    ...
    function buildNode(msg, pending = false) {
      ...
      `
        ...
        const nameEl = el('span.name', { dataset: { account } }, cachedName);
        const meta = el('div.meta', nameEl, el('time.time', time));
        // === 프로필 클릭 핸들러 연결 (옵션 제공 시만) ===
        if (typeof onProfileClick === 'function') {
            const activator = (ev) => onProfileClick(account, chatProfileService.getCached(account) ?? {
                nickname: cachedName,
                fetchedAt: Date.now(),
            }, ev);
        ...
        if (msg.attachments?.length) {
            const gallery = el('div.attachments', ...msg.attachments
    ...
    chatProfileService.addEventListener('chatprofilechange', (e) => {
        ...
        list.querySelectorAll(`.message .name[data-account="${account}"]`).forEach((node) => {
            node.textContent = profile.nickname ?? shortenAddress(account);
            const messageEl = node.closest('.message');
            const avatarContainer = messageEl?.querySelector('.avatar-container') ?? null;
            if (avatarContainer) {
    ...
    service.addEventListener('init', (e) => {
        const initialMessages = e.detail;
