...
function generateJazziconSVG(address, shapeCount = DEFAULT_SHAPE_COUNT, wobble = DEFAULT_WOBBLE, baseColors = DEFAULT_BASE_COLORS) {
    ...
    for (let i = 0; i < shapeCount; i++) {
        shapes.push('<rect x="0" y="0" width="100%" height="100%" transform="' +
            nextTransform(i) + '" fill="' + nextColor() + '" />');
    }
    return '<svg xmlns="http://www.w3.org/2000/svg" x="0" y="0" viewBox="0 0 100 100">' +
        shapes.join('') + '</svg>';
}
function createJazziconDataURL(address, shapeCount = DEFAULT_SHAPE_COUNT, wobble = DEFAULT_WOBBLE, baseColors = DEFAULT_BASE_COLORS) {
    return 'data:image/svg+xml;utf8,' +
        encodeURIComponent(generateJazziconSVG(address, shapeCount, wobble, baseColors));
}
function createJazzicon(address) {
